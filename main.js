...
function load(){
    ...
    function findNextPosition(currentPlayerPosition, currentPlayer){
        var coordinateToEvalue = currentPlayer.getCoordinateByPosition(currentPlayerPosition); // [1]
        for (var position of currentPlayer.listPositions) { //
            if (currentPlayerPosition != position) { // si la posicion a evaluar no es la misma 
                var coordinateByPosition = currentPlayer.getCoordinateByPosition( position);
